...
// const uint32_t TIMEOUT_us = 10000; // timeout time in milisecs?
const uint32_t TIMEOUT_us = 60000; // timeout time in milisecs?
...
class WaveModeListener : public ModeListener{
 public:
  WaveModeListener(){
    ...
    scroll_center_cw = KANDY_INC_WAVE_SEP_MIN;
    scroll_center_ccw = KANDY_DEC_WAVE_SEP_MIN;
...
// set wave sep back to 10 seconds 
if(mode == WAVE_MODE && !modes[mode]->locked &&
  velokey.scroll_right_down &&
  velokey.scroll_left_down){

 send_cmd(KANDY_RESET_WAVE_SEP);
}
