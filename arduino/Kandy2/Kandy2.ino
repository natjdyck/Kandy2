...
// Time of day clock
#define TOD_COLOR CRGB::Aqua 

// Set countdown
#define CD_SET_COLOR CRGB::Yellow

// Countdown wave 1 includes second 0 at the start
#define CD_START_COLOR CRGB::Yellow

// Countdown wave > 1 does not include second 0 at the start
#define CD_WAVES_COLOR CRGB::Yellow 

// Race clock not started dashes and colens
#define RC_OFF_COLOR CRGB::Red 

// Race clock
#define RC_COLOR CRGB::Green 

// Race clock during flashing green wave start --> set to black for no numbers includes "0" at the start of each wave
#define RC_START_COLOR CRGB::Black

// Set wave separation seconds
#define WAVE_SEP_SET_COLOR CRGB::Yellow

// Set wave
#define WAVE_SET_COLOR CRGB::Purple

// durations (seconds) of wave start block animations

// flashing green "GO" at start --> 9 max (if numbers do not display, check color above)
#define GO_TIME 5

// red countdown wave 1 start --> 59 max
#define WAIT_TIME 59
...
const uint32_t GO[1 * 16] = {
	0x783c,0xfc7e,0x1cee7,0x186c3,0x18603,0x18603,0x18603,0x18603,0x18603,0x186f3,0x186f3,0x186c3,0x186c3,0x1cee7,0xfc7e,0x783c, // GO 
};
...
void scrollGo(byte start, const struct CRGB & color) {
	
	fill(CRGB::Black);
	byte row, col;
	
	// cycle through all led in 16x18 grid and turn on or off if part off letter or not
	for (col = 0; col < 17; col++) {
		for (row = 0; row < 16; row++) {
		
			if ( (GO[row] >> col) & 1 ) {
				setPixel(row, col + start, color); // color led if part of letter
			} else {
				setPixel(row, col + start, 0); // black led if not part of letter
			}
...
void updateDisplay() {
	...
	if(mode == STANDBY_MODE) {
		...
		displayTime(race_hh % 100, race_mm, race_ss, CD_SET_COLOR, true);
		...
		if(racing) {
			...
			// countdwon color if before race start, race clock color after
			if(now.unixtime() > stopwatch_start_time) {
				displayTime(race_hh % 100, race_mm, race_ss, RC_COLOR, race_ss % 2);
			}
			else{
				displayTime(race_hh % 100, race_mm, race_ss, CD_WAVES_COLOR, race_ss % 2);
			}
			...
			if(pending_start) {
				...
				for(uint8_t i = 0; i < n_wave; i++) {
					...
					if(-(WAIT_WAVE_TIME + 1) <  wave_seconds && wave_seconds < 0) {

						if(race_ss > 0) {
							fill(CRGB::Red);
						}
						else {
							scrollGo(5, RC_COLOR); // this never happens?????
						}

						// black out behind digit
						for(int col=0; col<12; col++) { // # of columns wide
							for(int row=0; row<16; row++) {
								setPixel(row, col+22, CRGB::Black); // start at column #
							}
						...
						bigDigit(24, abs(wave_seconds), CD_WAVES_COLOR);
					...
					if(0 <= race_seconds - i * wave_sep && race_seconds - i * wave_sep < GO_TIME + 1) {
						...
						if (wave_seconds == 0 || wave_seconds % 2 == 0) {
							scrollGo(5, RC_COLOR);
						} else {
							scrollGo(34, RC_COLOR);
						}

						// displays seconds digits with green start blocks, unless rcStartColor is Black
						bigDigit(24, race_seconds - i * wave_sep, RC_START_COLOR);
			...
			draw_dashes(RC_OFF_COLOR);
			draw_colens(RC_OFF_COLOR);
